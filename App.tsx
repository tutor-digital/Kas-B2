...
// KONFIGURASI SUPABASE - SUDAH TERHUBUNG KE DATABASE ONLINE
...
const App: React.FC = () => {
  ...
  // Fungsi Mapping dari Database (snake_case) ke Frontend (camelCase)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setError("Gagal terhubung ke Cloud. Pastikan tabel database sudah siap.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // 3. Tambah Transaksi (Termasuk Otomatisasi Split 50/50)
  const handleAddTransaction = async (newTx: Omit<Transaction, 'id'>) => {
    const now = new Date().toISOString().split('T')[0];
    const baseId = Math.random().toString(36).substr(2, 9);
    
    try {
      if (newTx.type === TransactionType.INCOME && newTx.category === Category.DUES) {
        // Iuran Bulanan otomatis dibagi dua kantong
        const half = newTx.amount / 2;
        const payload = [
          { 
            id: baseId + '-a', 
            amount: half, 
            fund_category: FundCategory.ANAK, 
            description: `[Split] ${newTx.description}`, 
            date: newTx.date || now, 
          ...
          { 
            id: baseId + '-p', 
            amount: half, 
            fund_category: FundCategory.PERPISAHAN, 
            description: `[Split] ${newTx.description}`, 
            date: newTx.date || now, 
            recorded_by: 'Bendahara',
            type: newTx.type,
        ...
        const payload = { 
          ...
          date: newTx.date || now, 
          recorded_by: 'Bendahara', 
          fund_category: newTx.fundCategory,
          amount: newTx.amount,
  ...
  const stats = useMemo((): SummaryStats => {
    const calcFund = (fund: FundCategory) => {
       const inc = transactions
        .filter((t) => t.fundCategory === fund && t.type === TransactionType.INCOME)
        .reduce((sum, t) => sum + t.amount, 0);
       const exp = transactions
        .filter((t) => t.fundCategory === fund && t.type === TransactionType.EXPENSE)
        .reduce((sum, t) => sum + t.amount, 0);
       const base = fund === FundCategory.ANAK ? initialBalance.anak : initialBalance.perpisahan;
       return base + (inc - exp);
    };
    ...
    return {
      ...
      totalBalance: calcFund(FundCategory.ANAK) + calcFund(FundCategory.PERPISAHAN),
      balanceAnak: calcFund(FundCategory.ANAK),
      balancePerpisahan: calcFund(FundCategory.PERPISAHAN)
    };
        ...
        <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-100 px-8 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-2xl text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 transition-all font-bold text-xs uppercase tracking-widest"
            >
              <Wallet size={16} /> Atur Saldo
            </button>
            <button 
              onClick={() => setIsFormOpen(true)} 
              className="bg-slate-900 text-white px-6 py-2.5 rounded-2xl font-black flex items-center gap-2 shadow-lg text-xs uppercase tracking-widest transition-all active:scale-95"
            >
            ...
            <div className="p-8 space-y-8">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    type="number" 
                    value={initialBalance.anak} 
                    onChange={(e) => setInitialBalance({...initialBalance, anak: Number(e.target.value)})} 
                    className="w-full p-4 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-indigo-500 outline-none font-bold text-sm" 
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <input 
                    type="number" 
                    value={initialBalance.perpisahan} 
                    onChange={(e) => setInitialBalance({...initialBalance, perpisahan: Number(e.target.value)})} 
                    className="w-full p-4 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-indigo-500 outline-none font-bold text-sm" 
                  />
                </div>
              </div>
              ...
              <button 
                ...
                disabled={isSaving}
                className="w-full bg-indigo-600 text-white font-black py-4 rounded-2xl uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all shadow-lg"
              >
                {isSaving ? <Loader2 className="animate-spin" size={16} /> : <Check size={16} />}
                Update Saldo di Cloud
              </button>
            </div>
          </div>
        </div>
